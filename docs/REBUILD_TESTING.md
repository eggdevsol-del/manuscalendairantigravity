...
## 2.12.2 — restore the original custom checkout experience

The user's original DotsCheckout is the shared payment interface again. Checkout Sessions now use custom UI mode on the pinned Stripe API, rendered through CheckoutElementsProvider and the Payment Element in the existing dark form with its yellow payment button. Pro and Studio open in-app sheets instead of redirecting to hosted Checkout; pending legacy hosted subscription sessions are expired when starting the replacement checkout. Stripe's billing management portal remains a separate account-management action.

The form uses the provider total (including delivery and fees), collects required store contact information and delivery addresses, discloses monthly renewal, preserves inline declines/retry, and lets parent flows verify server-confirmed payment/entitlement state. PaymentIntent booking checkouts retain their existing integration. This supersedes the earlier embedded-interface correction: a Checkout Session secret needs its own provider, but does not require Stripe's full embedded checkout UI.

Validation: 100 existing/updated tests passed, plus three server-contract tests for Pro/Studio metadata and store delivery/Connect/reservation preservation. TypeScript, production frontend and bundled server builds passed. Deployed rendering and new subscription charge acceptance remain pending at this checkpoint. No new payment was submitted as part of these code checks.

Deployed acceptance: `/api/version` verified 2.12.2 at `082e5b43b99ebf721bc595ec7d0a3e6b16f916a9`. In-app Pro and Studio checkouts both rendered real Stripe test Payment Elements inside DotsCheckout, with A$99/A$499 totals and recurring-payment disclosure. The original hosted Pro session was replaced successfully. Stripe diagnostics confirmed session-derived amount/currency; remaining messages were product recommendations, not payment-form errors. No subscription was submitted.

`check-store-checkout.ts --run` passed against the deployed custom-mode session: exact provider/server total, stock held, real expiry webhook delivered, stock restored once, repeat cancellation safe, fixtures removed. Xcode simulator build passed for this release. Final visual polish changes Studio's action label to “Choose Studio” and gives the subscription sheet an opaque background so the underlying page headings do not show through; its production build passed. Final subscription payment/activation still requires the user to complete a test payment.
