...
async function main() {
  ...
  const userMatch = await db
    .select()
    .from(users)
    .where(eq(users.id, "user_ce549c0b4b6f90aaafad0546a0928050"));
  console.log(
    "Found in users table?",
    userMatch.length > 0 ? userMatch[0].name : "NO"
  );

  const clientMatch = await db
    .select()
    .from(clients)
    .where(eq(clients.id, "user_ce549c0b4b6f90aaafad0546a0928050"));
  console.log(
    "Found in clients table?",
    clientMatch.length > 0 ? clientMatch[0].name : "NO"
  );
