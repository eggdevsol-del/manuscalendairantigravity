    ...
    const checkStatus = async () => {
      ...
      if (isNative) {
        ...
        try {
          ...
          console.log("[useWebPush] Native status check:", {
            subscribed,
            subId,
          });
    ...
    async (options?: {
      ...
      try {
        ...
        if (result.success) {
          ...
          const failCount = result.results?.length
            ? result.results.length - sentCount
            : 0;

          if (sentCount > 0) {
            toast.success(
              `Successfully sent to ${sentCount} device${sentCount !== 1 ? "s" : ""}`
            );
          }
          if (failCount > 0) {
            toast.warning(
              `Failed to send to ${failCount} device${failCount !== 1 ? "s" : ""} (expired/invalid)`
            );
          }
          if (
            sentCount === 0 &&
