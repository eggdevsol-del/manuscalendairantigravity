...
import { FileText, HeartPulse, Save, ListIcon, ChevronLeft } from "lucide-react";
...
export function RegulationSettings({ onBack }: { onBack: () => void }) {
  ...
  const { data: templates, isLoading: templatesLoading } =
    trpc.forms.getTemplates.useQuery();
  const { data: logs, isLoading: logsLoading } =
    trpc.forms.getProcedureLogs.useQuery(undefined, {
  ...
  if (templatesLoading)
    return <LoadingState fullScreen message="Loading templates..." />;
  ...
  return (
    <div className="w-full h-full flex flex-col overflow-hidden relative">
      <div className="flex items-center gap-3 px-6 pt-6 pb-4 shrink-0 bg-transparent z-20 border-b border-white/5">
        ...
        <h2 className="text-xl font-semibold text-foreground">Regulation & Forms</h2>
      </div>
        ...
        <div className="pb-[180px] max-w-lg mx-auto space-y-6 px-4 pt-6">
          <div className="mb-2">
            <p className="text-muted-foreground text-sm font-medium">
              Configure templates and view procedure logs
            </p>
          </div>
          {/* Tab Switcher */}
