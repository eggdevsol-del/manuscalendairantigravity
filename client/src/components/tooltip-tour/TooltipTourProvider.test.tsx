...
beforeEach(() => {
  ...
  document.body.innerHTML =
    '<main data-tour-surface="Home"><h1>Home</h1><button>New booking</button><button id="save" data-tour-description="Review your changes before saving to keep business records accurate.">Save changes</button></main>';
});
afterEach(() => {
  cleanup();
  vi.restoreAllMocks();
...
const setup = () =>
  renderHook(() => useTooltipTour(), { wrapper: TooltipTourProvider });
it("reads every step without invoking a business control and persists only completion", async () => {
  ...
  await act(async () => {
    await hook.result.current.nextStep();
  });
...
it("follows newly opened feature sheets and returns to the page after closing", async () => {
  ...
  document.body.insertAdjacentHTML(
    "beforeend",
    '<section role="dialog"><h2>New booking</h2><label>Project completed by<input type="date"></label></section>'
  );
  await waitFor(() =>
  ...
  await waitFor(() =>
    expect(hook.result.current.activeTour?.steps[0].title).toBe("New booking")
  );
});
  ...
  act(() =>
    hook.result.current.startTour({
      ...
      steps: [
        ...
        { targetId: "css:#save", title: "Review", body: "Read" },
      ],
    })
  );
  act(() => {
    void hook.result.current.nextStep();
    hook.result.current.skipTour();
  });
  await act(async () => {
    resolve();
    await pending;
...
it("updates state-dependent guidance while preserving the highlighted control", async () => {
  ...
  await waitFor(() =>
    expect(hook.result.current.activeTour!.steps[1].body).toContain(
...
it("refreshes labels changed in place without rebuilding their DOM element", async () => {
  ...
  await waitFor(() =>
    expect(hook.result.current.activeTour!.steps[1].title).toBe(
      "Check confirmation"
    )
  );
  expect(hook.result.current.activeTour!.steps[1].targetId).toBe(target);
