...
import { Users, BarChart3, Mail, ShieldAlert, UserPlus, Trash2, User, Settings as SettingsIcon } from "lucide-react";
...
export default function StudioDashboard() {
    ...
    return (
        <PageShell>
            ...
            <div className="sticky top-0 z-10 px-4 pt-4 pb-2 bg-background/80 backdrop-blur-md border-b border-border/10">
                <div className="flex justify-between items-center w-full max-w-4xl mx-auto bg-white/5 rounded-xl p-1 relative border border-white/10 overflow-x-auto no-scrollbar">
                    ...
                    <button
                        ...
                        className={cn(
                            "flex-1 min-w-[100px] relative z-10 py-2.5 text-sm font-semibold transition-colors flex items-center justify-center gap-2",
                            activeTab === "invite" ? "text-primary-foreground" : "text-muted-foreground hover:text-white"
                        )}
                    >
                    ...
                    <button
                        ...
                        className={cn(
                            "flex-1 min-w-[100px] relative z-10 py-2.5 text-sm font-semibold transition-colors flex items-center justify-center gap-2",
                            activeTab === "settings" ? "text-primary-foreground" : "text-muted-foreground hover:text-white"
                        )}
                    >
            ...
            <div className={cn(tokens.contentContainer.base, "pb-24 pt-8 px-4 max-w-4xl mx-auto")}>
                        ...
                        >
                            ...
                            {teamMembers && teamMembers.length <= 1 && (
                                <Card className="p-6 bg-primary/10 border-primary/30">
                                    <h3 className="text-xl font-bold mb-2">Welcome to your Studio!</h3>
                                    <p className="text-muted-foreground mb-4">
                                        Your studio account is currently set up as an owner/manager hub.
                                        To start taking bookings, you should invite your personal Artist profile under the "Invite" tab.
                                    </p>
                            ...
                            <div className="space-y-6">
                                ...
                                <div>
                                    <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-4 pl-1 flex items-center gap-2">
                                        <Users className="w-4 h-4" /> Active Members ({activeMembers.length})
                                    </h3>
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        {activeMembers.map((member: any) => (
                                            <div key={member.id} className="p-4 rounded-xl bg-white/5 border border-white/10 flex items-center justify-between hover:bg-white/10 transition-colors">
                                                <div className="flex items-center gap-4">
                                                    ...
                                                    <div>
                                                        <p className="font-semibold text-lg text-foreground">{member.user.name}</p>
                                                        <div className="flex items-center gap-2 mt-0.5">
                                                            <span className="text-sm text-muted-foreground">{member.user.email}</span>
                                                            {member.role === 'owner' && <span className="px-2 py-0.5 rounded bg-amber-500/20 text-amber-500 text-[10px] font-bold uppercase tracking-wider">Owner</span>}
                                                            {member.role === 'manager' && <span className="px-2 py-0.5 rounded bg-blue-500/20 text-blue-500 text-[10px] font-bold uppercase tracking-wider">Manager</span>}
                                                        </div>
                                                    </div>
                                                </div>
                                ...
                                {pendingMembers.length > 0 && (
                                    <div className="pt-4">
                                        <h3 className="text-sm font-semibold text-muted-foreground uppercase tracking-wider mb-4 pl-1 flex items-center gap-2">
                                            <Mail className="w-4 h-4" /> Pending Invites ({pendingMembers.length})
                                        </h3>
                                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            {pendingMembers.map((member: any) => (
                                                <div key={member.id} className="p-4 rounded-xl bg-white/5 border border-white/5 border-dashed flex items-center justify-between opacity-70">
                                                    <div className="flex items-center gap-4">
                                                        ...
                                                        <div>
                                                            <p className="font-semibold text-lg text-foreground">{member.user.name || member.user.email}</p>
                                                            <div className="flex items-center gap-2 mt-0.5">
                                                                <span className="text-sm text-muted-foreground">Pending • Invited as {member.role}</span>
                                                            </div>
                                                        </div>
                                                    </div>
                            ...
                            <Card className="p-8 bg-gradient-to-br from-white/10 to-transparent border-white/20 shadow-xl">
                                <div className="mb-8 text-center">
                                    ...
                                    <p className="text-muted-foreground">
                                        Add artists and managers to your studio space.
                                ...
                                <form onSubmit={handleInvite} className="flex flex-col gap-6">
                                    <div className="space-y-4">
                                        <div className="space-y-2">
                                            <label className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">User Email</label>
                                            <Input
                                        ...
                                        <div className="space-y-2">
                                            <label className="text-sm font-semibold uppercase tracking-wider text-muted-foreground">Role</label>
                                            <select
                                            ...
                                            >
                                                <option value="artist">Artist (Takes bookings & manages clients)</option>
                                                <option value="manager">Manager (Admin access to studio)</option>
                                            </select>
                                        </div>
                                    </div>
                    ...
                    {activeTab === "analytics" && (
                        ...
                        >
                            <Card className="p-12 bg-gradient-to-br from-white/10 to-transparent border-white/20 text-center flex flex-col items-center justify-center min-h-[400px]">
                                ...
                                <h3 className="text-3xl font-bold mb-4 tracking-tight">Studio Analytics</h3>
                    ...
                    {activeTab === "settings" && (
                        ...
                        >
                            <Card className="p-8 bg-gradient-to-br from-white/10 to-transparent border-white/20 flex flex-col items-center justify-center min-h-[400px] text-center">
                                ...
                                <h3 className="text-3xl font-bold mb-4 tracking-tight">Studio Setup</h3>
                                <p className="text-muted-foreground text-lg max-w-md mx-auto mb-8 leading-relaxed">
                                    Manage your studio's billing, global policies, and booking details.
                                </p>
                                <Button
