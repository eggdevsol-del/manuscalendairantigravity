...
} from "@/components/chat/StudioInviteMessage";
import {
  ArrowLeft,
  Send,
  Zap,
...
export function ChatInterface({
        ...
        try {
          ...
          if (["accepted", "remittance_uploaded", "confirmed"].includes(meta.status)) {
            ...
            const dates = Array.isArray(meta.dates)
              ? meta.dates
              : Array.isArray(meta.proposedDates)
                ? meta.proposedDates
                : [];
            const lastDate =
  ...
  return (
    <div className={cn("flex flex-col h-full relative", className)}>
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-3 flex-1 min-w-0">
              ...
              <Button
                ...
                size="icon"
                className={cn(
                  "hover:bg-primary/10 -ml-2 shrink-0",
                  !onBack && "md:hidden"
                )}
                onClick={() =>
                  onBack ? onBack() : setLocation("/conversations")
                }
              >
            ...
            <div className="flex-1 min-w-0">
              ...
              {consultationData.preferredDate && (
                <p className="text-[10px] uppercase tracking-wider text-muted-foreground/80 mt-1 font-medium">
                  {new Date(
                    consultationData.preferredDate as any
      ...
      <div
        className={cn(
          "fixed left-4 right-4 z-30 transition-all duration-300 ease-spring",
            ...
            <Input
              ...
              onChange={e => {
                const dates = e.target.value
                  ...
                  .filter(Boolean);
                setClientConfirmDates(
                  dates.map(date => ({ date, selected: true }))
                );
              }}
            />
          </div>
          <Button className="w-full" onClick={() => handleClientConfirmDates()}>
            Confirm Dates
          </Button>
        </div>
      </BottomSheet>
