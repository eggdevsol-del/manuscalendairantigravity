      ...
      <div className="flex items-center justify-between mb-4">
        ...
        <span
          className={cn(
            ...
            tokens.proposalModal.badgeText,
            tokens.proposalModal.badgeRadius
          )}
        >
          {dateList.length} Sessions
        </span>
      </div>
      <div className="max-h-[160px] overflow-y-auto no-scrollbar space-y-3 pr-1">
        {dateList.map((dateStr, i) => (
          <div key={i} className="flex items-center gap-4">
            <div
              className={cn(
                "flex items-center justify-center text-muted-foreground font-bold text-xs",
                tokens.proposalModal.sessionBadge
              )}
            >
              #{i + 1}
            </div>
      ...
      <Accordion type="single" collapsible className="w-full">
        <AccordionItem value="cancellation" className="border-border">
          ...
          <AccordionContent className="text-muted-foreground px-2 pb-3 text-xs leading-relaxed">
            Deposits are non-refundable. Cancellations made within 48 hours of
            the appointment may forfeit the deposit. Please contact the artist
        ...
        <AccordionItem value="deposit" className="border-border">
          ...
          <AccordionContent className="text-muted-foreground px-2 pb-3 text-xs leading-relaxed">
            A deposit of ${depositAmount || 0} is required to secure these
            dates. The remaining balance of ${totalCost - (depositAmount || 0)}{" "}
            is due upon completion of the service.
          </AccordionContent>
        </AccordionItem>
      </Accordion>
    </Card>
        ...
        <>
          ...
          {!appliedPromotion && artistId && (
            <Button
              ...
              className={cn(
                "w-full",
                tokens.proposalModal.buttonHeight,
                tokens.proposalModal.buttonRadius,
                tokens.proposalModal.voucherButton
              )}
            >
          ...
          <div className="grid grid-cols-2 gap-3">
            <Button
              ...
              className={cn(
                tokens.proposalModal.buttonHeight,
                tokens.proposalModal.buttonRadius,
                tokens.proposalModal.declineButton
              )}
            >
              Decline
            </Button>
            <Button
              ...
              className={cn(
                ...
                tokens.proposalModal.buttonHeight,
                tokens.proposalModal.buttonRadius,
                tokens.proposalModal.acceptButton
              )}
            >
      ...
      {isArtist && status === "pending" && (
        <div className="col-span-2 flex flex-col gap-2">
          <Button
            variant="destructive"
            className={cn(
              "w-full opacity-60 hover:opacity-100",
              tokens.proposalModal.buttonHeight,
              tokens.proposalModal.buttonRadius
            )}
            onClick={onRevoke}
  ...
  return (
    ...
    >
      <DialogPrimitive.Portal>
        ...
        <DialogPrimitive.Content className="app-viewport fixed inset-0 z-[101] w-full h-[100dvh] outline-none flex flex-col justify-end overflow-hidden data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:slide-in-from-bottom-10 data-[state=open]:slide-in-from-bottom-0">
          <div
            className={cn(
              "w-full h-full flex flex-col px-0 overflow-hidden relative mt-0 md:mt-4",
              tokens.proposalModal.sheetBg,
              tokens.proposalModal.sheetBlur,
              tokens.proposalModal.sheetRadius,
              tokens.proposalModal.sheetShadow
            )}
          >
            <div
              className={cn(
                "absolute top-0 inset-x-0 h-px pointer-events-none",
                tokens.proposalModal.highlightGradient,
                tokens.proposalModal.highlightOpacity
              )}
            />
            <div className="absolute top-6 right-6 z-30">
              <Button
                ...
                className={cn(
                  "text-foreground",
                  tokens.proposalModal.closeButton
                )}
                onClick={onClose}
              >
                <X className="w-5 h-5" />
              ...
              {showVoucherList ? (
                ...
                <div className="pb-32 max-w-lg mx-auto space-y-4">
                  <div className="mb-6 px-2">
                    ...
                    <DialogTitle
                      className={cn(
                        "mb-6 pr-12 line-clamp-2",
                        tokens.proposalModal.title
                      )}
                    >
                      {serviceName}
                    </DialogTitle>
                    <div className="w-full">
                      <div className="flex flex-wrap items-center justify-between gap-y-4 gap-x-2">
                        ...
                        <div className="flex items-center gap-3">
                          <span className="text-2xl font-bold text-foreground tracking-tight">
                            {hours}
                            <span className="text-lg font-normal text-muted-foreground/60 ml-0.5">
                              h
                            </span>
                            {minutes > 0 && (
                              <span className="ml-1">
