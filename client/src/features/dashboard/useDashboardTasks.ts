...
import {
  DashboardTaskRegister,
  CHALLENGE_TEMPLATES,
} from "./DashboardTaskRegister";
...
export function useDashboardTasks() {
  ...
  const [config, setConfig] = useState<DashboardConfig>(
    DashboardConfigRegister.getConfig()
  );
    ...
    if (state.lastVisitDate !== today) {
      ...
      if (state.activeChallengeId) {
        ...
        const template = CHALLENGE_TEMPLATES.find(
          (t: ChallengeTemplate) => t.id === state.activeChallengeId
        );
        if (template) {
  ...
  const markDone = useCallback((taskId: string) => {
    setState(prev => {
      ...
      const newStreak =
        task.domain === "social" ? prev.socialStreak + 1 : prev.socialStreak;
  ...
  const stopChallenge = useCallback(() => {
    ...
    DashboardEventLogger.log("challenge_stopped", {
      templateId: state.activeChallengeId,
    });
  ...
  // Update comms pref
  const setCommsPlatform = useCallback(
    (platform: "ios" | "android" | "desktop") => {
      DashboardConfigRegister.updateConfig({
        comms: { ...config.comms, platform },
      });
