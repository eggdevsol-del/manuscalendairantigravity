...
export function AppTour() {
    ...
    // Only run tour for artists who haven't completed onboarding yet
    useEffect(() => {
        if (typeof window === "undefined") return;

        // Give the DOM a tiny fraction of a second to render the layout before starting the tour
        if (user && user.role === "artist" && user.hasCompletedOnboarding === 0) {
    ...
    if (!user || user.role !== "artist" || user.hasCompletedOnboarding !== 0) {
        return null; // Don't mount at all if they are already onboarded or a client
    }
    ...
    const handleJoyrideCallback = (data: CallBackProps) => {
        ...
        if (finishedStatuses.includes(status)) {
            setRun(false);
            // We intentionally DO NOT complete onboarding here.
            // Onboarding is only completed when they finish the Dashboard Checklist.
        }
    };
