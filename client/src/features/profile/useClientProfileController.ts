...
export function useClientProfileController() {
  ...
  const { data: profile, isLoading: loadingProfile } =
    trpc.clientProfile.getProfile.useQuery(input);
  const { data: spend, isLoading: loadingSpend } =
    trpc.clientProfile.getSpendSummary.useQuery(input);
  const { data: history, isLoading: loadingHistory } =
    trpc.clientProfile.getHistory.useQuery(input);
  const { data: boards, isLoading: loadingBoards } =
    trpc.clientProfile.getBoards.useQuery(input);
  const { data: photos, isLoading: loadingPhotos } =
    trpc.clientProfile.getPhotos.useQuery(input);
  const { data: upcoming, isLoading: loadingUpcoming } =
    trpc.clientProfile.getUpcoming.useQuery(input);
  const { data: forms, isLoading: loadingForms } =
    trpc.clientProfile.getConsentForms.useQuery(input);
