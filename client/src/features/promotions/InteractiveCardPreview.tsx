...
export function InteractiveCardPreview({
  ...
  const handleMouseMove = (e: React.MouseEvent) => {
    ...
    const newX = Math.min(
      100,
      Math.max(0, (data.backgroundPositionX || 50) - deltaX * sensitivity)
    );
    const newY = Math.min(
      100,
      Math.max(0, (data.backgroundPositionY || 50) - deltaY * sensitivity)
    );
  ...
  const handleWheel = (e: React.WheelEvent) => {
    ...
    const newScale = Math.min(
      3,
      Math.max(0.5, (data.backgroundScale || 1) + delta)
    );
    onUpdateScale(newScale);
  };
  ...
  return (
    <div className="flex flex-col items-center gap-4">
      <div
        ...
        title={
          data.backgroundImageUrl
            ? "Drag to move background, Pinch or Scroll to zoom"
            : "Preview"
        }
      >
        <PromotionCard
