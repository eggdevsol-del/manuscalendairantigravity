...
export function PromotionWizardContent({
  onClose,
  onSuccess,
  initialData,
  onStepChange,
}: PromotionWizardContentProps) {
  const { user } = useAuth();
  ...
  const canProceed = () => {
    ...
    switch (step) {
      case "type":
        return true;
      case "value":
        return !!value && parseFloat(value) > 0 && !!name.trim();
      default:
        return true;
    }
  };
      ...
      {types.map(t => {
        ...
        return (
          <motion.div
            ...
            className={cn(
              fab.itemRow,
              "cursor-pointer active:scale-95 transition-transform px-1"
            )}
            onClick={() => {
              onSelect(t.id);
              goNext();
          ...
          >
            <div className="flex-1 min-w-0 flex flex-col items-end">
              <p className={cn(fab.itemLabel, "font-bold text-foreground")}>
                {t.title}
              </p>
      ...
      <motion.div variants={fab.animation.item} className="space-y-2">
        <div className={cn(card.base, card.bg, "p-3")}>
          <p className="text-[9px] leading-relaxed text-muted-foreground font-medium italic">
            "Your new {previewData.type} template is ready. Once created, you
