    ...
    <div className="mb-6">
      ...
      >
        ...
        <CardContent className="p-0 pl-2">
          {" "}
          {/* Add pl-2 to account for the border if not absolute overlap, but it is absolute. So maybe standard padding. */}
          ...
          <div className="bg-secondary/30 p-3 flex gap-2">
            <Link href={`/chat/${client.id}`} className="flex-1">
              <Button
                variant="secondary"
                className="w-full gap-2 h-10 shadow-sm bg-background/80 hover:bg-background"
              >
                <MessageCircle className="w-4 h-4" />
                Message
              </Button>
            </Link>
            <Button className="flex-1 gap-2 h-10 shadow-sm">Check In</Button>
          </div>
        </CardContent>
      </Card>
    </div>
