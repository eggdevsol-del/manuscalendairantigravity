...
export function useOriginalCalendarController() {
  const { user } = useAuth();
  ...
  const isSoloArtistView = isArtistLike; // Include the owner’s pre-studio and personal bookings.
  const {
  ...
  const appointments = useMemo(() => {
    if (isArtistLike)
      return [
        ...new Map(
          [...(studioAppointments || []), ...(soloAppointments || [])].map(
