...
export function MonthBreakdown({
  ...
  return (
    <div className="absolute top-0 left-0 right-0 z-0 pt-11 px-0 pb-6 flex flex-col w-full h-[55dvh]">
      <div className="px-6 mb-4">
        <h2 className="text-xl font-light text-muted-foreground uppercase tracking-widest">
          My Month
        </h2>
      </div>
        ...
        <div className="w-full px-2">
          ...
          <div className="flex flex-col gap-0.5">
            {weeks.map((week, weekIndex) => (
              <div
                key={weekIndex}
                className="grid grid-cols-[30px_repeat(7,1fr)] gap-0.5"
              >
                {/* Week Number */}
                    ...
                    >
                      ...
                      {validBackgrounds.length > 0 && (
                        <div className="absolute inset-0 flex flex-row">
                          {validBackgrounds.map((bgClass, idx) => (
                            <div
                              key={idx}
                              className={cn("flex-1 h-full", bgClass)}
                            />
                          ))}
                        </div>
                      )}
                      ...
                      <div className={cn(contentClass, "bg-transparent z-10")}>
                        <span
                          className={
                            validBackgrounds.length > 0
                              ? "mix-blend-overlay text-white drop-shadow-md font-bold"
                              : ""
                          }
                        >
        ...
        >
          ...
          <div className="text-center">
            <div className="text-xl font-medium text-foreground">
              {isClient ? clientTotalBookingsThisMonth : freeDatesCount}
            </div>
            <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mt-1">
              {isClient ? "Total Bookings" : "Dates Free"}
            </div>
          ...
          <div className="text-center">
            <div className="text-xl font-medium text-[var(--color-success)]">
              ...
              }).format(totalRemainingBalance)}
            </div>
            <div className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mt-1">
              {isClient ? "Remaining Bal" : "Expected Revenue"}
            </div>
            <div className="text-[9px] text-muted-foreground/60">
              (this month)
            </div>
