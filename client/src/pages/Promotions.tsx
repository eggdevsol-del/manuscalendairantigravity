...
export default function Promotions() {
  ...
  const activeCard =
    selectedCard || (viewMode === "swipe" ? filteredCards[focalIndex] : null);
      ...
      <div className="relative flex-1 flex flex-col overflow-hidden h-full">
        ...
        <div
          className={cn(
            ...
            isTeaserClient && "filter blur-sm pointer-events-none"
          )}
        >
                      ...
                      >
                        <SwipeableCardWrapper
                          ...
                          onSwipe={direction => {
                            if (
                              direction === "up" &&
                              focalIndex < filteredCards.length - 1
                            ) {
        ...
        <Dialog open={showDeleteDialog} onOpenChange={setShowDeleteDialog}>
          <DialogContent>
            <DialogHeader>
              ...
              <DialogDescription>
                Permanently delete "{activeCard?.name}"? This action cannot be
                undone.
              </DialogDescription>
            </DialogHeader>
            <DialogFooter>
        ...
        {showSendSheet && activeCard && (
          <SendPromotionSheet
            ...
            promotion={activeCard as PromotionCardData}
          />
        )}
...
function EmptyState({
  ...
  const Icon =
    type === "voucher" ? Gift : type === "discount" ? Percent : CreditCard;
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      ...
      {isArtist && (
        <Button onClick={onCreate} variant="default">
          <Plus className="w-4 h-4 mr-2" /> Create {defaults.labelSingular}
        </Button>
      )}
    </div>
  );
...
function AutoApplySheet({
  ...
  const [isAutoApply, setIsAutoApply] = useState(
    promotion.isAutoApply || false
  );
