...
export default function ArtistProfileTab() {
  ...
  // Fetch own portfolio
  const { data: portfolio = [] } = trpc.portfolio.list.useQuery(undefined, {
    staleTime: 30000,
  });
      ...
      const timer = setTimeout(() => {
        ...
        const tour = {
          ...
          steps: PROFILE_ONBOARDING_TOUR.steps.map((step, i) => ({
            ...
            ...(i === 0
              ? { onNext: () => setEditMode(true), nextDelay: 400 }
              : {}),
          })),
        };
        startTour(tour);
  ...
  const bulkDeleteMutation = trpc.portfolio.bulkDelete.useMutation({
    onSuccess: data => {
      ...
      utils.feed.getArtistPublicProfile.invalidate();
      toast.success(
        `${data.deletedCount} item${data.deletedCount !== 1 ? "s" : ""} deleted`
      );
  ...
  const handleAvatarUpload = useCallback(
    async (e: React.ChangeEvent<HTMLInputElement>) => {
      ...
      try {
        ...
        formData.append(
          "file",
          await (await fetch(resized)).blob(),
          "avatar.jpg"
        );
      ...
      <div className="artist-profile-header">
        {inFeedView ? (
          <button
            ...
            onClick={() => setFeedViewImageId(null)}
          >
            back
          </button>
        ) : (
        ...
        <span className="artist-profile-header-name">
          {profile?.displayName || user?.name || "Profile"}
        </span>
        {inFeedView ? (
          <div style={{ width: 40 }} />
        ) : editMode ? (
                    ...
                    <>
                      <input
                        ...
                        style={{
                          ...
                          fontWeight: 700,
                          background:
                            "var(--color-bg-secondary, rgba(255,255,255,0.06))",
                          border: "1px solid var(--color-border)",
                          borderRadius: 10,
                          padding: "8px 14px",
                          color: "var(--foreground)",
                  ...
                  {!editMode && profile.keywords.length > 0 && (
                    <div className="artist-profile-keywords">
                      {profile.keywords.map((kw, i) => (
                        <span key={i} className="artist-profile-keyword">
                          {kw}
                        </span>
                      ))}
                    </div>
                      ...
                      {selectMode ? (
                        <>
                          ...
                          <button
                            ...
                            style={{
                              ...
                              fontWeight: 600,
                              cursor:
                                selectedIds.size > 0 ? "pointer" : "default",
                            }}
                          >
                            {bulkDeleting ? (
                        ...
                        <>
                          <button
                            ...
                            onClick={() =>
                              setLocation("/settings?section=instagram")
                            }
                            style={{
                              display: "flex",
                              alignItems: "center",
                ...
                {portfolio.length === 0 ? (
                  ...
                  >
                    ...
                    <div>
                      ...
                      >
                        Import your work from Instagram to get started, or
