...
export function useFunnelController(artistSlug: string) {
  ...
  const [bodyPlacementImages, setBodyPlacementImages] = useState<
    UploadedImage[]
  >([]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      const referenceUrls = await processImages(
        referenceImages,
        setReferenceImages
      );
      const placementUrls = await processImages(
        bodyPlacementImages,
  ...
  const canProceed = useCallback(() => {
    switch (currentStep) {
      ...
      case 3:
        return true; // Optional placement
      case 4:
