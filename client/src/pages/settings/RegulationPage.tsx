...
import { FileText, HeartPulse, Scale, Save } from "lucide-react";

type TabType = 'form9' | 'medical' | 'consent';
...
export default function RegulationPage({ onBack }: { onBack: () => void }) {
    ...
    const { data: templates, isLoading } = trpc.forms.getTemplates.useQuery();
    ...
    if (isLoading) return <LoadingState fullScreen message="Loading templates..." />;
    ...
    return (
        <PageShell>
            ...
            <div className="px-6 pt-4 pb-8 z-10 shrink-0 flex flex-col justify-center h-[15vh] opacity-80">
                <p className="text-3xl font-light text-foreground/90 tracking-tight">Legal Assets</p>
                <p className="text-muted-foreground text-sm font-medium mt-1">Configure templates for client signing</p>
            </div>
                        ...
                        <div className="flex gap-1 bg-white/5 p-1 rounded-2xl border border-white/5">
                            {tabs.map((tab) => (
                                <button
                                    ...
                                    className={cn(
                                        "flex-1 flex items-center justify-center gap-2 py-2.5 px-3 rounded-xl transition-all font-medium text-xs",
                                        activeTab === tab.id
