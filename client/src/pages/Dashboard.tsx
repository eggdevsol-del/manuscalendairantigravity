...
import { MoneyTab } from "@/features/dashboard/MoneyTab";
...
const TITLES = ["Home", "Clients", "Money", "Supplies"];
...
export default function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Steps 0-2: Home, 3-5: Clients, 6-7: Supplies
    let targetIndex = 0;
    if (currentStep >= 3 && currentStep <= 5) targetIndex = 1;
    else if (currentStep >= 6) targetIndex = 3; // Supplies is now index 3
    ...
    <PageShell>
      ...
      <div className={cn(tokens.contentContainer.base, "relative")}>
        ...
        <div className="flex-1 overflow-y-auto mobile-scroll">
          <motion.div 
            animate={{ marginTop: activeCategory === "supplies" ? 0 : -8 }}
          ...
          >
            ...
            {user?.role === "artist" || user?.role === "admin" ? (
              <motion.div
                animate={{
                  height: (activeCategory === "supplies" || activeCategory === "money") ? 0 : "auto",
                  opacity: (activeCategory === "supplies" || activeCategory === "money") ? 0 : 1,
                  scale: (activeCategory === "supplies" || activeCategory === "money") ? 0.95 : 1,
                }}
                transition={{ type: "spring", stiffness: 300, damping: 30 }}
                className="overflow-hidden"
              >
                {/* Payout widget is now in MoneyTab — keep minimal summary here */}
              </motion.div>
            ) : null}
          </motion.div>
