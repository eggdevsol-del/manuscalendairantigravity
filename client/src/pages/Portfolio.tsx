    ...
    () => (
      <BottomNavRow>
        <Button
          variant="ghost"
          size="sm"
          className="flex-col h-auto py-2 px-3 gap-1 hover:bg-transparent min-w-[70px] snap-center shrink-0 transition-all duration-300 relative text-muted-foreground opacity-70 hover:opacity-100"
        >
        ...
        >
          <div className="relative">
            <Trash2 className="w-6 h-6 mb-0.5" />
          </div>
        ...
        <div className="flex-1 w-full h-full px-4 pt-4 overflow-y-auto mobile-scroll touch-pan-y">
          <div className="grid grid-cols-2 gap-4 pb-32 max-w-lg mx-auto">
            {portfolioItems?.map(item => (
              <div
                key={item.id}
                className="relative group aspect-square rounded-2xl overflow-hidden bg-muted"
              >
                <img
                  src={item.imageUrl}
              ...
              <div className="col-span-2">
                <Empty className="py-10">
                  <EmptyHeader>
                    ...
                    <EmptyTitle>
                      {isArtist ? "No images yet" : "No portfolio items"}
                    </EmptyTitle>
                    <EmptyDescription>
                      {isArtist
                        ? "Upload some work to showcase your talent!"
