...
export async function reportError(
  ...
  try {
    ...
    await fetch("/api/trpc/errorLog.log", {
      ...
      body: JSON.stringify({
        json: {
          message: "Client runtime error",
          stack: telemetryFrames(error.stack),
          componentStack: telemetryFrames(extra?.componentStack),
          boundary:
            extra?.boundary === "window.onerror" ||
            extra?.boundary === "unhandledrejection"
              ? extra.boundary
              : "react",
          url: telemetryRoute(window.location.href),
