  ...
  try {
    if (Capacitor.isNativePlatform()) {
      ...
      if (NativeOneSignal) {
        ...
        console.warn(
          "[OneSignal] Native plugin not found. Falling back to CRM Web SDK."
        );
        await initWebOneSignal();
      }
    } else {
      console.log(
        "[OneSignal] Initializing Web SDK for SMS/Email CRM ONLY. Push is handled by VAPID."
      );
      await initWebOneSignal();
    }
  ...
  try {
    await Promise.race([
      ...
      new Promise<void>((_, reject) =>
        setTimeout(() => reject(new Error("Web SDK init timed out")), 5000)
      ),
    ]);
...
export async function requestNotificationPermission(): Promise<boolean> {
  try {
    ...
    if ("Notification" in window && !Capacitor.isNativePlatform()) {
      ...
      console.log(
        "[PushManager] Native Browser Web Prompt result:",
        nativeWebPerm
      );
      ...
      if (NativeOneSignal) {
        return await Promise.race([
          ...
          new Promise<string | null>(resolve =>
            setTimeout(() => {
              console.warn(
                "[OneSignal] Native getPushSubscriptionId timed out"
              );
              resolve(null);
            }, 2000)
          ),
...
export async function isSubscribed(): Promise<boolean> {
  try {
    ...
    console.error(
      "[OneSignal/PushManager] Failed to check subscription status:",
      error
    );
    return false;
  }
