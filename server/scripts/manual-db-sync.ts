...
async function runMigration() {
  ...
  try {
    await connection.query(
      "ALTER TABLE `studios` ADD `publicSlug` varchar(100);"
    );
    await connection.query(
      "ALTER TABLE `studios` ADD `funnelEnabled` tinyint DEFAULT 0;"
    );
  ...
  try {
    ...
    await connection.query(
      "UPDATE `studios` SET `publicSlug` = CONCAT('studio-', id) WHERE `publicSlug` IS NULL;"
    );
  ...
  try {
    ...
    await connection.query(
      "ALTER TABLE `messages` MODIFY COLUMN `messageType` enum('text','system','appointment_request','appointment_confirmed','image','video','studio_invite') NOT NULL;"
    );
