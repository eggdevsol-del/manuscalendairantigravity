...
async function generateFollowUpTasks(
  ...
  for (const consult of respondedConsultations) {
    ...
    tasks.push({
      ...
      clientId: consult.clientId,
      clientName: consult.client?.name || null,
      actionType: 'in_app', // Default to in_app, but UI shows secondary email/sms
      smsNumber: consult.client?.phone || null,
      smsBody: consult.client?.phone ? `Hi ${consult.client?.name || 'there'}! Just following up on your consultation. Let me know if you have any questions!` : null,
...
async function generateStaleConversationTasks(
  ...
  for (const conv of conversations) {
    ...
    tasks.push({
      ...
      smsBody: conv.client?.phone ? `Hi ${conv.client?.name || 'there'}! Just checking in since we haven't chatted in a while.` : null,
