  ...
  try {
    ...
    if (existingEndpoint) {
      if (existingEndpoint.userId === userId) {
        ...
        console.log(
          `[PushService] Reassigned subscription from user ${existingEndpoint.userId} to ${userId}`
        );
        return { success: true, subscriptionId: existingEndpoint.id };
      }
    } else {
    ...
    console.error(
      `[PushService] Subscription failed for user ${userId}:`,
      error
    );
    throw error;
  }
...
export async function unsubscribeFromPush(
  ...
  try {
    ...
    console.error(
      `[PushService] Unsubscribe failed for user ${userId}:`,
      error
    );
    throw error;
  }
...
export async function getUserSubscriptions(userId: string) {
  ...
  try {
    ...
    console.error(
      `[PushService] Failed to get subscriptions for user ${userId}:`,
      error
    );
    return [];
  }
...
export async function sendPushNotification(
  ...
  for (const sub of subs) {
    try {
      ...
      console.log(
        `[PushService] Sent notification to user ${userId}, subscription ${sub.id}`
      );
    } catch (error: any) {
      console.error(
        `[PushService] Push failed for subscription ${sub.id}:`,
        error
      );
...
export async function broadcastToUsers(
  ...
  for (const userId of userIds) {
    try {
      ...
      console.error(
        `[PushService] Broadcast failed for user ${userId}:`,
        error
      );
      totalFailed++;
    }
  ...
  console.log(
    `[PushService] Broadcast complete: ${totalSent} sent, ${totalFailed} failed`
  );
...
export async function removeExpiredSubscription(
  ...
  try {
    ...
    console.error(
      `[PushService] Failed to remove subscription ${subscriptionId}:`,
      error
    );
  }
}
