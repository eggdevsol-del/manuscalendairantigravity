...
export const studiosRouter = router({
   ...
   * Matches client requested style against resident artists' specialties,
   * then routes to the qualified artist with the most available chair capacity (lowest near-term utilization).
   */
  routeStudioIntake: protectedProcedure
    .input(
      z.object({
        studioId: z.string().optional(),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      // Fetch all active resident artists
      const members = await db.query.studioMembers.findMany({
        where: and(
          eq(schema.studioMembers.studioId, studio.id),
          eq(schema.studioMembers.status, "active")
      ...
      const rankedArtists = await Promise.all(
        members.map(async (m) => {
          const user = await db.query.users.findFirst({ where: eq(schema.users.id, m.userId) });
          const settings = await db.query.artistSettings.findFirst({ where: eq(schema.artistSettings.userId, m.userId) });

          // Check style match
          const artistStyles = (settings?.keywords || "").toLowerCase() + " " + (settings?.styleOptions || "").toLowerCase();
          const targetStyle = (input.stylePreference || "").toLowerCase().trim();
          const isStyleMatch = targetStyle.length > 0 ? artistStyles.includes(targetStyle) : true;

          // Compute working schedule capacity
          let weeklyCapacityHours = 0;
          if (settings?.workSchedule) {
            try {
              const rawSchedule = typeof settings.workSchedule === "string" ? JSON.parse(settings.workSchedule) : settings.workSchedule;
              if (Array.isArray(rawSchedule)) {
                rawSchedule.forEach((d: any) => {
                  if (d.enabled !== false) {
                    const s = d.start || d.startTime || "10:00";
              ...
              console.error("Error parsing schedule for intake routing:", err);
            }
          }
          if (weeklyCapacityHours <= 0) weeklyCapacityHours = 35;
          const monthlyCapacityHours = Math.round(weeklyCapacityHours * 4.333);

          // Get active booked hours
          const activeAppts = await db.query.appointments.findMany({
            where: and(
              eq(schema.appointments.artistId, m.userId),
              gte(schema.appointments.startTime, thirtyDaysAgo),
          ...
          const bookedHours = activeAppts.reduce((hrs, a) => {
            ...
            const e = a.endTime ? new Date(a.endTime).getTime() : s + 3 * 3600000;
            if (isNaN(s) || isNaN(e) || e <= s) return hrs + 2;
            return hrs + Math.min(12, Math.max(0.5, (e - s) / 3600000));
          }, 0);

          const freeHours = Math.max(0, monthlyCapacityHours - Math.round(bookedHours));
          const utilizationPct = Math.min(100, Math.max(0, Math.round((bookedHours / monthlyCapacityHours) * 100)));

          return {
            userId: m.userId,
            name: user?.name || "Resident Artist",
            avatarUrl: user?.avatarUrl,
            specialties: settings?.keywords || "Custom Tattoo",
            isStyleMatch,
            monthlyCapacityHours,
            bookedHours: Math.round(bookedHours),
            freeHours,
      ...
      // Sort by style match first, then by highest free hours (lowest utilization)
      const sorted = rankedArtists.sort((a, b) => {
        if (a.isStyleMatch && !b.isStyleMatch) return -1;
        if (!a.isStyleMatch && b.isStyleMatch) return 1;
      ...
      return {
        recommendedArtist: recommended,
        allCandidates: sorted,
        explanation: recommended
          ? `Matched to ${recommended.name} based on ${recommended.isStyleMatch ? `style specialisation (${input.stylePreference || "Custom"})` : "overall availability"} and highest near-term chair capacity (${recommended.freeHours}h available, ${recommended.utilizationPct}% booked).`
          : "No resident artists available",
      };
    }),
});
