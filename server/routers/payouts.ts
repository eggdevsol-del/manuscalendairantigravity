...
import { paymentLedger, artistSettings, users } from "../../drizzle/schema";
import { stripe } from "../services/stripe";
import {
  previewArtistRefund,
  requestArtistRefund,
} from "../services/artistRefund";

export const payoutsRouter = router({
  /**
   * Refund Transaction — Reverses a Stripe payment and logs it
   */
  refundPreview: artistProcedure
    .input(z.object({ ledgerId: z.number().int().positive() }))
    .query(({ ctx, input }) =>
      previewArtistRefund(ctx.user.id, input.ledgerId)
    ),
  ...
  nextPayout: artistProcedure.query(async ({ ctx }) => {
    ...
    try {
      ...
      const currency =
        account.default_currency || (account.country === "NZ" ? "nzd" : "aud");
  ...
  earningsBreakdown: artistProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      // They never moved through the platform and have zero fees.
      const conditions = [
        eq(paymentLedger.artistId, ctx.user.id),
        notInArray(paymentLedger.paymentMethod, ["cash", "bank"]),
      ];
      if (startDate) {
