...
export const dashboardTasksRouter = router({
   ...
   */
  getBusinessTasks: protectedProcedure.query(async ({ ctx }) => {
    const { user } = ctx;
    ...
    const tasks = await generateBusinessTasks(
      db,
      user.id,
      settings.maxVisibleTasks || 10
    );
  ...
  startTask: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const [result] = await db.insert(schema.activeTasks).values({
        ...
        priorityLevel:
          input.priorityScore >= 800
            ? "critical"
            : input.priorityScore >= 500
              ? "high"
              : input.priorityScore >= 300
                ? "medium"
                : "low",
  ...
  completeTask: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      const timeToCompleteSeconds = Math.floor(
  ...
  updateSettings: protectedProcedure
    .input(
      z.object({
        ...
        preferredEmailClient: z
          .enum(["default", "gmail", "outlook", "apple_mail"])
          .optional(),
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.maxVisibleTasks !== undefined)
        updateData.maxVisibleTasks = input.maxVisibleTasks;
      if (input.goalAdvancedBookingMonths !== undefined)
        updateData.goalAdvancedBookingMonths = input.goalAdvancedBookingMonths;
      if (input.preferredEmailClient !== undefined)
        updateData.preferredEmailClient = input.preferredEmailClient;
      if (input.showWeeklySnapshot !== undefined)
        updateData.showWeeklySnapshot = input.showWeeklySnapshot ? 1 : 0;
   ...
   */
  getWeeklySnapshot: protectedProcedure.query(async ({ ctx }) => {
    const { user } = ctx;
    if (user.role !== "artist" && user.role !== "admin") {
      throw new TRPCError({ code: "FORBIDDEN" });
    }
    ...
    const avgCompletionTime =
      totalTasks > 0
        ? Math.round(
            completions.reduce((sum, c) => sum + c.timeToCompleteSeconds, 0) /
              totalTasks
          )
        : 0;
    ...
    const avgTier1Time =
      tier1Completions.length > 0
        ? Math.round(
            tier1Completions.reduce(
              (sum, c) => sum + c.timeToCompleteSeconds,
              0
            ) / tier1Completions.length
          )
        : 0;

    // Get consultation response times
    const consultationTasks = completions.filter(
      c => c.taskType === "new_consultation"
    );
    const avgConsultationResponse =
      consultationTasks.length > 0
        ? Math.round(
            consultationTasks.reduce(
              (sum, c) => sum + c.timeToCompleteSeconds,
              0
            ) / consultationTasks.length
          )
        : 0;

    // Calculate benchmarks comparison
    const responseTimeVsBenchmark =
      avgConsultationResponse > 0
        ? Math.round(
            (BENCHMARKS.AVERAGE_RESPONSE_TIME / avgConsultationResponse) * 100
          )
        : 100;
    ...
    if (
      avgConsultationResponse > 0 &&
      avgConsultationResponse < BENCHMARKS.EXCELLENT_RESPONSE_TIME
    ) {
      efficiencyScore += 15;
    } else if (
      avgConsultationResponse > 0 &&
      avgConsultationResponse < BENCHMARKS.GOOD_RESPONSE_TIME
    ) {
      efficiencyScore += 10;
    }
    ...
    return {
      ...
      comparison: {
        ...
        benchmarkLabel:
          responseTimeVsBenchmark >= 150
            ? "Elite"
            : responseTimeVsBenchmark >= 120
              ? "Excellent"
              : responseTimeVsBenchmark >= 100
                ? "Good"
                : responseTimeVsBenchmark >= 80
                  ? "Average"
                  : "Needs Improvement",
      },
      efficiencyScore,
   ...
   */
  dismissWeeklySnapshot: protectedProcedure.mutation(async ({ ctx }) => {
    const { user } = ctx;
    if (user.role !== "artist" && user.role !== "admin") {
      throw new TRPCError({ code: "FORBIDDEN" });
    }
   ...
   */
  shouldShowWeeklySnapshot: protectedProcedure.query(async ({ ctx }) => {
    const { user } = ctx;
    if (user.role !== "artist" && user.role !== "admin") {
...
function generateInsights(
  ...
  if (avgResponseTime > 0) {
    if (avgResponseTime < BENCHMARKS.ELITE_RESPONSE_TIME) {
      insights.push(
        "🚀 Your consultation response time is elite! You're 3x faster than the average business."
      );
    } else if (avgResponseTime < BENCHMARKS.EXCELLENT_RESPONSE_TIME) {
      insights.push(
        "⚡ Great response times! You're responding faster than 80% of businesses."
      );
    } else if (avgResponseTime > BENCHMARKS.AVERAGE_RESPONSE_TIME) {
      insights.push(
        "💡 Tip: Responding to consultations within 1 hour can increase conversions by 7x."
      );
    }
  }
  ...
  if (tier1Tasks >= 5) {
    insights.push(
      "💰 Strong revenue protection! You completed " +
        tier1Tasks +
        " critical tasks this week."
    );
  } else if (tier1Tasks === 0 && totalTasks > 0) {
    insights.push(
      "📋 Focus on Tier 1 tasks (red) first - they protect your immediate revenue."
    );
  }
  ...
  if (efficiencyScore >= 90) {
    insights.push(
      "🏆 You're performing at elite level! Keep up the excellent work."
    );
  } else if (efficiencyScore >= 70) {
    insights.push(
      "📈 Good progress! A few more completed tasks could push you to excellent."
    );
  } else {
    insights.push(
      "🎯 Try to complete at least 5 tasks per week to improve your efficiency score."
    );
  }
