  ...
  scrapeShopifyStore: protectedProcedure
    ...
    .mutation(async ({ input }) => {
      ...
      try {
            ...
            while (wooPage <= MAX_PAGES) {
              ...
              const mappedWooProducts = wData.map((wp: any) => {
                const priceVal = wp.prices?.price
                  ? (parseInt(wp.prices.price, 10) / 100).toString()
                  : "0";
                return {
                  id: wp.id.toString(),
                  title: wp.name,
        ...
        if (allProducts.length === 0) {
          throw new Error(
            "Could not find public products at this URL. Make sure it is a valid Shopify or modern WooCommerce store without strict API blocking."
          );
        }
        ...
        for (const p of allProducts) {
          ...
          for (const ev of existingVariants) {
            if (!seenVariantIds.has(ev.id)) {
              await db
                .delete(schema.supplierProductVariants)
        ...
        for (const ep of existingProducts) {
          if (!seenProductIds.has(ep.id)) {
            await db
              ...
              .where(eq(schema.supplierProducts.id, ep.id));
          }
        }
