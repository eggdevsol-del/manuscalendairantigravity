...
async function verifyAndFixDatabase() {
  ...
  try {
    ...
    console.log(
      `[DB Verify] Found ${tableNames.length} existing tables:`,
      tableNames.join(", ")
    );
    ...
    if (!fs.existsSync(drizzlePath)) {
      ...
      if (!fs.existsSync(drizzlePath)) {
        console.error(
          "[DB Verify] Drizzle migrations not found at:",
          drizzlePath
        );
        process.exit(1);
      }
    ...
    for (const file of migrationFiles) {
      ...
      console.log(
        `[DB Verify] Found ${statements.length} statements in ${file}`
      );
      ...
      console.log(
        `[DB Verify] ${file}: ${successCount} success, ${skipCount} skipped, ${errorCount} errors`
      );
    }
    ...
    const finalTableNames = (finalTables as any[]).map(
      (t: any) => t.table_name || t.TABLE_NAME
    );
    ...
    const stillMissing = expectedTables.filter(
      t => !finalTableNames.includes(t)
    );
    if (stillMissing.length > 0) {
      console.error(
        `[DB Verify] ⚠ Still missing ${stillMissing.length} tables:`,
        stillMissing.join(", ")
      );
