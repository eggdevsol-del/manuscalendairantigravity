...
export async function initializeDatabase() {
  ...
  if (!databaseUrl) {
    console.warn(
      "[DB Init] DATABASE_URL not set, skipping database initialization"
    );
    return;
  }
  ...
  try {
    ...
    if (tablesExist) {
      console.log(
        "[DB Init] Tables already exist, checking for new migrations..."
      );
      ...
      if ((promoTable as any)[0].count === 0) {
        ...
        const promoMigration = path.join(
          drizzlePath,
          "0012_promotions_system.sql"
        );
    ...
    if (!fs.existsSync(drizzlePath)) {
      console.error(
        `[DB Init] Drizzle migrations directory not found: ${drizzlePath}`
      );
      return;
    }
    ...
    for (const file of migrationFiles) {
      ...
      console.log(
        `[DB Init] Executing ${statements.length} statements from ${file}...`
      );
        ...
        if (statement.length > 0) {
          try {
            ...
            if (
              ...
              console.log(
                `[DB Init] Skipping duplicate/existing item in ${file} (${i + 1}/${statements.length})`
              );
              continue;
            }
            ...
            console.error(
              `[DB Init] Error in ${file} statement ${i + 1}:`,
              error.message
            );
    ...
    const tableNames = (finalTables as any[]).map(
      (t: any) => t.table_name || t.TABLE_NAME
    );
    console.log(
      `[DB Init] ✓ Successfully created ${tableNames.length} tables:`,
      tableNames.join(", ")
    );
  } catch (error: any) {
