...
async function audit() {
  try {
    ...
    await sysLogger
      .info("audit", "Targeted audit for 3PM/4PM issue")
    ...
    const targetAppts = await db
      .select()
      .from(appointments)
      .where(or(eq(appointments.id, 219), eq(appointments.id, 220)));

    console.log(`\nAppointments Found: ${targetAppts.length}`);
    targetAppts.forEach(a => {
      console.log(
        `ID: ${a.id}, Title: ${a.title}, Start: ${a.startTime}, Status: ${a.status}`
      );
    });
    ...
    const relatedMessages = allMessages.filter(m => {
      ...
      try {
        ...
        return (
          meta.serviceName === "1 hour" ||
          (meta.dates &&
            meta.dates.some((d: string) => d.includes("2026-02-17")))
        );
      } catch (e) {
        return false;
      }
    ...
    relatedMessages.forEach(m => {
      try {
        ...
        console.log(
          `Dates: ${JSON.stringify(meta.dates || meta.proposedDates)}`
        );
      } catch (e) {
