...
try{for(const width of [320,390,820]){
 ...
 const summaryMin=await proposal.evaluate(el=>parseFloat(getComputedStyle(el).minHeight));
 assert(compactHeight>=summaryMin&&compactHeight<=summaryMin+4,`Compact proposal height: ${compactHeight}`);
 ...
 await page.locator('.simple-project-link').first().click();
 await page.getByRole('list',{name:'Project sittings'}).getByRole('button',{name:/Sitting /}).first().click();
 ...
 await page.getByRole('list',{name:'Project sittings'}).waitFor();
 await page.goto(base+'/projects/12?session=101');
 detail=page.getByRole('dialog',{name:/Sitting \d+ ·/});await detail.waitFor();
 assert.equal(await detail.getByRole('button',{name:/Request a date change|Request cancellation/}).count(),0);
 await detail.getByRole('link',{name:/^Message /}).click();
